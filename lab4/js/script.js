...
// ===== М'яка валідація: порожні поля + число для expense =====
function validateAnimalForm(form) {
  ...
  // м'яка перевірка числа (дозволяємо кому)
  ...
  // Додатково дозволимо браузеру підсвітити інші HTML-правила (required/number)
...
function renderAnimals(filterText = "", sortExpensive = false) {
  ...
  // сортування за витратами (спадання)
  if (sortExpensive) {
    animals.sort((a, b) => Number(b.expense || 0) - Number(a.expense || 0));
  }

  // очистити список (прибере хардкод з HTML)
  list.innerHTML = "";
    ...
    li.innerHTML = `
      ...
      <div class="animal-card__body">
        <div class="animal-card__content">
          ...
          <time class="animal-card__last-update" datetime="${
            animal.updatedAt || ""
          }">
            Last updated ${timeAgo(animal.updatedAt)}
          </time>
...
function handleCreateForm() {
  ...
  // НЕ відключаю браузерну валідацію — працюють HTML-атрибути
  ...
  form.addEventListener("submit", (e) => {
    ...
    const safeExpense = parseFloat(form.expense.value.replace(",", ".")); // вже перевірено
